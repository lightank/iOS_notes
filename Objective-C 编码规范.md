...
```objc
Project Name
├── AppDelegate // 主要用于存放AppDelegate及其分类(用于三方库的配置)
│   ├── AppDelegate + AppService    // 大部分的配置内容
│   ├── AppDelegate + UMeng // 单个三方的配置，最后在AppService中调用
│   └── AppDelegate + ...
├── Modules //按APP模块划分
│   ├── Home
│   │   ├── Controller
...
**Tips**：在`Objective-C`中，清晰比简洁更重要。由于 Xcode 强大的自动补全功能，我们不必担心名称过长的问题。
...
整个工程的命名风格要保持一致性，最好和苹果SDK的代码保持统一。不同类中完成相似功能的方法应该叫一样的名字，比如我们总是用`count`来返回集合的个数，不能在 A 类中使用`count`而在 B 类中使用`getNumber`。
...
如果多人开发，可以在新建`class`、`Category`时加上自己的大写英文字符前缀，前缀全部使用大写，英文字符在3个及其以上
...
方法一般以小写字母打头，每一个后续的单词首字母大写，方法名中不应该有标点符号（**包括下划线**），有几个例外：
...
```objc
Download_Progressbar_Highlighted@2x.png
Download_Progressbar_Normal@2x.png
```
...
一元运算符与操作数之前没有空格。如：`-1`
二元算数运算符两侧应该空格有空格。如`1 + 1`

## 括号的换行

关于括号是不是要换行，一直是程序员们乐此不疲争论的一个梗

将带有分支的代码（`if`、`else`、`switch`、`while` ...）和上一个分支的结束括号（`)`、`}`），以及自己分支的开始括号（`(`、`{`）放在同一行，来保证代码的紧凑可读。

```objc
// 个人对方式1与方式2都能接受，但1个人写的代码应该要保持一致
// 方式1
if (user.isHappy)
{
    // ...
...
推荐的代码组织方式是将`dealloc`方法放在实现文件的最前面（直接在`@synthesize`以及`@dynamic`之后），`init`方法应该跟在`dealloc`方法后面。
...
你的方法可能要求一些参数来满足特定的条件（比如不能为`nil`），在这种情况下最好使用 `NSAssert()` 来断言条件是否成立或是抛出一个异常。
...
虽然我们知道这样写很丑, 但是我们应该要在我们的`category`方法前加上自己的小写前缀以及下划线，比如`- (id)lt_myCategoryMethod`。 这种实践同样被苹果推荐。

这是非常必要的。因为如果在扩展的 category 或者其他 category 里面已经使用了同样的方法名，会导致不可预计的后果。实际上，实际被调用的是最后被加载的那个`category`中方法的实现。

一个好的实践是在`category`方法名中使用前缀。
...
分类可以用来在头文件中定义一组功能相似的方法。这是在`Apple`的`Framework`也很常见的一个实践（下面例子的取自`NSDate`头文件）。我们也强烈建议在自己的代码中这样使用。
...
* 性质一致的方法用一个`#pragma mark`
* 不同功能组的方法多个`#pragma mark`
* protocols 的实现
* 对父类方法的重写
...
## 设计模式
## ReactiveCocoa + MVVM 
推荐使用[ReactiveCocoa][ReactiveCocoa] + `MVVM`设计代码架构，只有代码架构上清晰合理了上述规则才能锦上添花。
...
---
# 参考链接
*	[Coding Guidelines for Cocoa](https://developer.apple.com/library/mac/documentation/Cocoa/Conceptual/CodingGuidelines/CodingGuidelines.html) by [Apple][Apple Documentation Archive]
...
* [App Programming Guide for iOS](https://developer.apple.com/library/archive/documentation/iPhone/Conceptual/iPhoneOSProgrammingGuide/Introduction/Introduction.html) by [Apple][Apple Documentation Archive]
* [Objective-C-Coding-Guidelines-In-Chinese](https://github.com/QianKaiLu/Objective-C-Coding-Guidelines-In-Chinese) from [GitHub][Github]
* [搜狐视频iOS团队 Objective-C 编码规范](https://github.com/SOHOVideo/Objective-C-Coding-Guidelines)
